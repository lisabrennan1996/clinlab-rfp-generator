...
ana_rows = _build_analytes()                       # [[test, comment], ...]
...
soa_n = rebuild_soa(doc, 7, soa_visits, soa_rows)
...
nref = fill_spec(18, 'LTS PK', referral, single_col=True)
nsto = sum(fill_spec(20, c, storage, single_col=False) for c in ('LTS DNA', 'LTS Serum', 'LTS Plasma'))
nsto += fill_spec(22, 'LTS RNA', storage, single_col=False)
...
N_VISITS = len(soa_visits)          # visit count from the reconstructed SoA
...
def _fill_row_ccs(table, label_kw, value):
    for r in table.rows:
        if label_kw.lower() in r.cells[0].text.lower():
            ...
            return True
    return False
_fill_row_ccs(doc.tables[0], 'Immuno Oncology protocol', 'Yes' if IS_ONCOLOGY else 'No')

def _find_table(kw):
    for t in doc.tables:
        if kw.lower() in t.rows[0].cells[0].text.lower():
            return t
    return None
